...
void movement()
{
	...
	for (int i = 0; i < g.n; i++){
		...
		//Modify to check for nearest object
		//before checking for collision
		//after introducing box array.
		Shape *s = &g.box;
		
		if (abs(p->s.center.y - s->center.y) < s->height){
	
      		        if (abs(p->s.center.x - s->center.x) < s->width){
			p->velocity.y = -p->velocity.y/2;
			p->s.center.y += (p->s.center.y - s->center.y);
			}
		}
		...
		if (p->s.center.y <  0.0 || p->s.center.y > g.yres) {
			p->velocity.y = -p->velocity.y/2;
		}
		if (p->s.center.x > g.xres) {
			cout << "exited" << endl;
